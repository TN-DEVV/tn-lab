...
Padlock.Close = function() {
    ...
    $.post('https://qb-cokelab/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://qb-cokelab/PadLockClose');
    if (data.error != null) {
        $.post('https://qb-cokelab/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://qb-cokelab/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://qb-cokelab/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://qb-cokelab/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
