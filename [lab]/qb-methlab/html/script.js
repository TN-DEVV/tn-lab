...
Padlock.Close = function() {
    ...
    $.post('https://qb-methlab/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://qb-methlab/PadLockClose');
    if (data.error != null) {
        $.post('https://qb-methlab/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://qb-methlab/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://qb-methlab/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://qb-methlab/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
